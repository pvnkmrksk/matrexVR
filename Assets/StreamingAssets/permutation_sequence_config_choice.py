    ...
    if insert_isi and varying_isi_length:
        ...
        if len(data['sequences'])==len(isi_list['sequences']):
            ...
            for this_trial,this_isi in zip(data['sequences'],cycle(isi_list['sequences'])):
                print(this_trial)
                this_config_file = this_trial['parameters']['configFile']
                with open(this_config_file , "r") as f:
                    #print(f"load trial sequence from file {this_trial}")
                    this_trial_variables = json.loads(f.read())

                    this_rotational_gain=this_trial_variables['closedLoopOrientation']
                    this_translational_gain=this_trial_variables['closedLoopPosition']
                #print(this_translational_gain)
                original_file_name=this_isi['parameters']['configFile']
                file_name_split=original_file_name.split('.')
                #print(this_isi['parameters'])
                #print(this_isi)
                #this_isi['parameters'].update({'configFile': f"{file_name_split[0]}_{this_translational_gain}_{this_rotational_gain}.{file_name_split[1]}"})
                immu_dict = ImmuDictionary(sceneName=this_isi['sceneName'],duration=this_isi['duration'],parameters=Parameters(configFile=f"{file_name_split[0]}_{this_translational_gain}_{this_rotational_gain}.{file_name_split[1]}"))
                #print(immu_dict)
                #print(data.parameters.configFile)
                #new_sequences.append(this_isi)
                new_sequences.append(asdict(immu_dict))
                new_sequences.append(this_trial)
    else:
