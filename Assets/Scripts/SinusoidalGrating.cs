...
//                 // Map the sinusoidal value to a color between color1 and color2
//                 float t = s * amplitude + 0.5f;
...
//         // Apply the changes to the texture and assign it to the material
//         texture.Apply();
    ...
    void FillTexture()
    {
        for (int x = 0; x < textureWidth; x++)
        {
            for (int y = 0; y < textureHeight; y++)
            {
                ...
                // float t = saturate(s * amplitude + 0.5f);
                // Color c = Color.Lerp(color1, color2, t);
                // Raise the sinusoidal value to a higher power
                // float t = Mathf.Pow(s * amplitude + 0.5f, power);
                // Color c = Color.Lerp(color1, color2, t);

                // Use a conditional expression to check the level
                // float t =
                //     level < 0.5f
                //         ? saturate(s * level * 2 + 0.5f)
                //         : smoothstep(0.5f, 0.5f, s * (level - 0.5f) * 2 + 0.5f);
                // Color c = Color.Lerp(color1, color2, t);
